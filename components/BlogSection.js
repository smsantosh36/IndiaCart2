  ...
  return (
    <section className="mb-12">
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-display text-2xl font-medium">Blog</h2>
        <Link
          href="/blog"
      ...
      <div className="relative group">
        {/* mobile: 2.5 visible (peek hint ke liye — pata chale scroll ho sakta hai), desktop (md): 4 visible */}
        <div
          ref={scrollRef}
        ...
        >
          {latest.map((blog) => (
            <div
              key={blog.id}
              className="flex-none snap-start w-[calc((100%-20px)/2.5)] md:w-[calc((100%-60px)/4)]"
            >
              <BlogCard blog={blog} />
            </div>
          ))}
